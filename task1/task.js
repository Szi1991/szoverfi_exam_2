export function getLongestBookByAuthorName(authors, books, authorName) {
    ...
    const autor = authors.find((author) => author.name === authorName); 

    if (!autor) 
        return null;
  
    
    const booksAutor = books.filter((book) => book.author_id === autor.id);
    if (booksAutor.length === 0) 
        return null;
  
    const longestBook = booksAutor.sort((a, b) => {
      if (b.pages === a.pages) {
